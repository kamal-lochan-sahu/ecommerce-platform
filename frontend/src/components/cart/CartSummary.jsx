...
export default function CartSummary({ subtotal, onCheckout, compact = false }) {
  ...
  const applyCoupon = async () => {
    ...
    setLoading(true);
    try {
      const res = await cartService.applyCoupon(coupon.trim().toUpperCase());
      setApplied(res.data.coupon);
      toast.success(`Coupon applied! ${res.data.coupon.percent}% off 🎉`);
    } catch {
      // Mock coupon for dev
  ...
  return (
    <div className={clsx("space-y-4", !compact && "card p-5")}>
      ...
      {!applied ? (
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2.5 text-sm rounded-xl border border-gray-200
                         focus:outline-none focus:ring-2 focus:ring-primary-500 uppercase"
            />
          </div>
          <button
            onClick={applyCoupon}
            disabled={loading || !coupon}
            className="px-4 py-2.5 bg-primary text-white text-sm font-medium rounded-xl
                       hover:bg-primary-600 disabled:opacity-50 transition-all"
          >
            {loading ? "..." : "Apply"}
          </button>
        </div>
        ...
        <div className="flex items-center justify-between bg-green-50 border border-green-200
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-green-600">{applied.percent}% off applied!</span>
          </div>
      ...
      <div className="space-y-2.5 text-sm">
        ...
        {delivery === 0 && (
          <p className="text-xs text-green-600">🎉 Free delivery on this order!</p>
        )}
        {subtotal < 499 && delivery > 0 && (
