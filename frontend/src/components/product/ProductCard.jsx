...
function Stars({ value = 0, count = 0 }) {
  return (
    <div className="flex items-center gap-1">
      <div className="flex">
        {[1,2,3,4,5].map(s => (
          <svg key={s} className={clsx("w-3 h-3", s <= Math.round(value) ? "text-amber-400" : "text-gray-200")}
...
export default function ProductCard({ product }) {
  ...
  const {
    _id, name, slug, brand,
    price, comparePrice,
    images, ratings, stock, discount,
  } = product;

  // Handle both mock (number) and real (object) ratings
  const ratingValue = typeof ratings === "object" ? ratings?.average || 0 : ratings || 0;
  const ratingCount = typeof ratings === "object" ? ratings?.count  || 0 : 0;
      ...
      <div className="card-hover overflow-hidden">
        ...
        <div className="relative aspect-square overflow-hidden bg-gray-50">
          ...
          <div className="absolute top-2 left-2 flex flex-col gap-1">
            ...
            {isOutOfStock && <Badge variant="gray">Out of Stock</Badge>}
          </div>
        ...
        {/* Info */}
        <div className="p-3">
          {brand && <p className="text-xs text-gray-400 uppercase tracking-wide mb-0.5">{brand}</p>}
          <h3 className="text-sm font-medium text-gray-900 line-clamp-2 mb-1.5 group-hover:text-primary transition-colors">
            {name}
          </h3>
          <Stars value={ratingValue} count={ratingCount} />
